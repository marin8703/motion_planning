...
namespace d_hold
{
	...
	unsigned int map_size = 101;
	
	//which obstacle map to use
	int map_choice = 2;//2 is like 3 in images
	
	//Size of the back step in certain situations
	unsigned int step_back = 2;
	...
	int interpolation_degree = 2;	//stick with 2 seems to work best
...
std::vector<std::vector<int> > generate_xy_map(float current_pos_[], float goal_pos_[])
{
	...
	for(unsigned int i = 0; i <= d_hold::map_size; i++)
	{
		...
		{		
			if(i == 50)	//50, and not 51, because we start at 0 in the c++ array numbering convention
			{
				if(k >= 0 && k <= 100)
				{
					obs_map[i][k] = 8;
	...
	obs_map[50][0] = 6; //initial position, using the ROS coordinate convention, centered at 50 (being 0 x, 0 y in this case)
...
//		<< d_hold::current_u_x_y[2] << "\n";
	...
	do
	{
//		if(count == 10)		//to limit loop in debug
//			d_hold::end = true;
...
int motion_info()
{
	...
	float i_u = 5.0;	//when to get this data from curve
	...
	std::vector<float> path_xy_i1(3,0.0);
	std::vector<float> path_xy_i2(3,0.0);
	std::vector<float> path_xy_i3(3,0.0);
	...
	//motion variables
	float tangential_velocity = 0.0, tangential_velocity_modified = 0.0;	//augmented is the adjusted velocity if the results of the initial are outside the constrains
	float normal_acceleration = 0.0, normal_acceleration_modified = 0.0;
	float rho_gain = 0.0;
	float max_desired_vel = 50.00, max_desired_norm_acc = 18.0;	// in m/s and m/s^2, about 110 mph and 1.835 gs
	float v_path_max = 0.0;
		
	
/*	//use if i_u parameter is given
	do
	{
		std::cout << "Enter parameter to evaluate (0:" << ( d_hold::t_storage[int (d_hold::t_storage.size()-1)] - 0.1) << "):" << std::endl;	//randomly choose 0.1 to keep input below last know
		std::cin >> i_u;
	}while(i_u > ( d_hold::t_storage[int (d_hold::t_storage.size()-1)] - 0.1) || i_u < 0);	//repeat until the specified values is in proper range	
	path_xy_i1 = evaluate_path_point(i_u);
*/	
	//use if x, y point is provided
	float x_pos=0.0, y_pos=0.0;
	std::cout << "Enter desired x position:"  << std::endl;	//randomly choose 0.1 to keep input below last know
	std::cin >> x_pos;
	std::cout << "Enter desired y position:"  << std::endl;	//randomly choose 0.1 to keep input below last know
	std::cin >> y_pos;
	
	std::vector<float> temp_xy(2,0.0);
	for(float i_u_d= 0.0; i_u_d < d_hold::t_storage[int (d_hold::t_storage.size()-1)]; i_u_d = i_u_d + 0.05)
	{
		temp_xy= evaluate_path_point(i_u_d);
		ROS_INFO("motion info: @ %f, xy position= (%f, %f)...", i_u, temp_xy[0], temp_xy[1]);
		if(temp_xy[0]+0.5 >= x_pos && temp_xy[0]-0.5 <= x_pos)
		{
			if(temp_xy[1]+0.5 >= y_pos && temp_xy[1]-0.5 <= y_pos)
			{
				path_xy_i1 = temp_xy;
				i_u = i_u_d;
				break;
	...
	path_xy_i2 = evaluate_path_point(i_u+dt);
	path_xy_i3 = evaluate_path_point((i_u+dt)+dt);
	
	ROS_INFO("motion info: @ %f, 1:xy position= (%f, %f)...", i_u, path_xy_i1[0], path_xy_i1[1]);
	ROS_INFO("motion info: @ %f, 2:xy position= (%f, %f)...", i_u, path_xy_i2[0], path_xy_i2[1]);
	ROS_INFO("motion info: @ %f, 3:xy position= (%f, %f)...", i_u, path_xy_i3[0], path_xy_i3[1]);
	
	for(int i = 0; i < 3; i++)
	{
		c_dot[i] = path_xy_i2[i] - path_xy_i1[i];
		c_dot_2[i] = path_xy_i3[i] - path_xy_i2[i];
	...
	rho = 1.0 / k;
	ROS_INFO("motion info: @ %f, k= %f, rho= %f...", i_u, k, rho); 
	
	//velocity information
	rho_gain = rho*0.01;	//should be less that 1.0 nearly at all times, bud doesnt matter, initial guess
	tangential_velocity = max_desired_vel*rho_gain;
	
	v_path_max = tangential_velocity_modified = sqrt(tangential_velocity * rho);
	if(tangential_velocity >= v_path_max)
	{
		if(max_desired_vel <= v_path_max)
		{
			tangential_velocity_modified = max_desired_vel;
			normal_acceleration_modified = pow(tangential_velocity_modified,2) / rho;
			ROS_INFO("motion info: @ %f, !SLOW DOWN! normal_acc reached/exceeded...", i_u);
		}
		else if(max_desired_vel > v_path_max)
		{
			tangential_velocity_modified = v_path_max;
			normal_acceleration_modified = pow(tangential_velocity_modified,2) / rho;
			ROS_INFO("motion info: @ %f, !SLOW DOWN! normal_acc reached/exceeded...", i_u);
		}
	}
	else if(tangential_velocity < v_path_max)
	{
		tangential_velocity_modified = sqrt(max_desired_norm_acc * rho);
		normal_acceleration_modified = pow(tangential_velocity_modified,2) / rho;
		if(tangential_velocity_modified >= max_desired_vel)
		{
			tangential_velocity_modified = max_desired_vel;
			normal_acceleration_modified = pow(tangential_velocity_modified,2) / rho;
	...
	ROS_INFO("motion info: @ %f, tan_velocity= %f, tangential_velocity_modified= %f, rho_gain= %f...", i_u, tangential_velocity, tangential_velocity_modified, rho_gain); 
	ROS_INFO("motion info: @ %f, tan_acc= %f, morm_acc_modified= %f...", i_u, normal_acceleration, normal_acceleration_modified); 
...
int obstacles_Callback()//const laser_obstacle_id::ObstaclesConstPtr& edges_xy_)
{
	...
	float initial_position[2] = {0.0,0.0}; 
	float final_position[2] = {72.1, 76.6}; // for map 1 {93.1, 67.6}, map 2/3 {72.1, 76.6}
...
//		return -1; //Error: current position or goal position is on or out of grid boundary
	...
	std::vector<std::vector<int> > obstacle_map = generate_xy_map(initial_position, final_position); //old function is generate_map
...
//	return 1;	//for debug just to set up map	
